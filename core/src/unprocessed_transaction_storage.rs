...
pub enum UnprocessedTransactionStorage {
    ...
    BundleStorage(BundleStorage), // TODO (LB): add bundle storage here
}
...
impl UnprocessedTransactionStorage {
    ...
    where
        F: FnMut(
            &[(DeserializedBundlePackets, SanitizedBundle)],
            &mut BundleStageLeaderStats,
        ) -> Vec<usize>,
    {
        match self {
    ...
    pub(crate) fn insert_bundles(
        ...
        match self {
            UnprocessedTransactionStorage::BundleStorage(bundle_storage) => {
                bundle_storage.insert_bundles(deserialized_bundles)
            }
...
impl BundleStorage {
        ...
        ) -> Vec<usize>,
    {
        ...
        let response = processing_function(&sanitized_bundles, bundle_stage_leader_stats);
    ...
    fn drain_and_sanitize_bundles(
        &mut self,
        bank: Arc<Bank>,
        _bundle_stage_loop_stats: &mut BundleStageLoopStats,
        bundle_stage_leader_stats: &mut BundleStageLeaderStats,
        blacklisted_accounts: &HashSet<Pubkey>,
    ) -> Vec<(DeserializedBundlePackets, SanitizedBundle)> {
